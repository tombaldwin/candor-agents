...
check("spec envelope (candor.spec = 0.16)", rep["candor"]["spec"] == "0.16")
...
with tempfile.TemporaryDirectory() as td:
    ...
    check("observe: spec 0.16 envelope + hash + package",
          obs["candor"]["spec"] == "0.16" and by["session"]["hash"] == "fixture#session" and obs["package"] == "fixture")
...
check("--agents prints the version header + the exact installed contract",
      r.returncode == 0 and r.stdout.startswith("<!-- candor-agents 0.16")
...
check("scan --gate-json <violating>: verdict {spec:0.16, ok:false} agrees with exit 1",
      rgv.returncode == 1 and _gv["spec"] == "0.16" and _gv["ok"] is False, json.dumps(_gv))
...
check("scan --gate-json <clean policy>: ok:true, violations [], exit 0",
      ...
      and json.load(open(_gj2)) == {"spec": "0.16", "ok": True, "violations": []})
...
_gate_blocked = {"spec": "0.16", "ok": False, "violations": [
...
_report = {"candor": {"version": "0.8.7", "toolchain": "candor-java 0.8.7", "spec": "0.16"},
...
_lg2 = _mkd()
_gp2 = _wjson(_lg2, "gate.json", {"spec": "0.16", "ok": True, "violations": []})
