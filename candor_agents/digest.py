...
def render(summary, project, title=None, allowed=(0, [])):
    ...
    if blocked:
        ...
        # A change blocked WITHOUT a policy code = the edit-loop stopped a new-effect introduction. Itemize
        # it too, so the bullets account for every caught change (not just the policy ones) and the header
        # count reconciles — else "2 caught" over one AS-EFF line leaves the reader asking about the other.
        nc = s.get("blockedNoCode", 0)
        if nc:
            effs = s.get("blockedGained") or []
            tail = f" ({', '.join(effs)})" if effs else ""
            L.append(f"  - {nc}× introduced a new effect{tail} not in the baseline — blocked for review")
    else:
    ...
    if s.get("hasReviewMs"):
        # Count only the TIMED checks — a jar/CI record has no reviewMs (null), so claiming "across N
        # checks" over all turns would imply timings we never took. Report what was actually measured.
        timed = s.get("timedChecks", turns)
        L.append("")
        L.append(f"_candor's own time this period: {s['candorMs'] / 1000:.1f}s across {timed} timed "
                 f"check{'' if timed == 1 else 's'}._")
    L.append("")
